...
class CorrectGrid(object):
	...
	'''
	def __init__(self, lat:float, lon:float, day:int, api_var:str):
		'''
		:param lat: 预报地点纬度
		:param lon: 预报地点经度
		:param day: 预报的天数
	...
	def request_api(self)->(list):
		...
		url = host + '/weather/forecast?lon=%s&lat=%s&hrs=%s&features=[%s]'%(self.lon, self.lat, (self.day-1)*24, self.api_var)
		try:
	...
	def gfs_forcastdata(self, forecastWeather, var_name)->(list, list):
		'''
...
if __name__ == '__main__':
	...
	for province, city, lat, lon, height in city_list:
		grid = CorrectGrid(lat, lon, 15, 'tmax,tmin,apcp')
